...
//angularJS
  ...
  horseApp.config(function ($routeProvider, $compileProvider) {
    $routeProvider
      ...
      // 联系客服页面
      .when("/contactService",{
        templateUrl:'tpl/contactService.html',
      ...
      //剩余畅行无忧数量页面
      .when("/cxwyServer",{
        templateUrl:'tpl/cxwyServer.html',
        controller:"cxwyServerCtrl"
      })
      //订单列表页面
      ...
      //原厂更换轮胎页面
      .when("/originalChangeTire/:cityId",{
  ...
  horseApp.controller('horseMainCtrl',function($scope,$http,$rootScope,$location){
    ...
    //登陆刷新数据时间戳
    var userDataTimer=localStorage['userDataTimer'];
    ...
    $scope.$watch("$viewContentLoaded",function(){
      ...
      if(userName&&userPwd){
        $http({
          ...
          data:"phone="+userName+"&password="+strmd5(userPwd)+"&init_time="+userDataTimer
        }).success(function(data){
          console.log(data);
          if(data.code=="E0000"){
            commFinish();
            ...
            localStorage['userDataTimer']=data.data.init_time;
            // 对用户是否进行车辆设置进行判断
  ...
  horseApp.controller('loginCtrl',function($scope,$rootScope,$http,$location){
    ...
    $scope.wxlogin=function(){
      //开始通讯加载
      commStart();
      //微信登陆后为保证用户下单信息的完整性对帐号的手机号进行设置， 跳转userSettingTel页面进行用户手机设置
      //检查微信是否安装
      Wechat.isInstalled(function (installed) {
        ...
        if(WEIXININSTALL){
          ...
          Wechat.auth(wxScope, function (response) {
              ...
              success:function(data){
                ...
                $.ajax({
                  ...
                  success:function(userData){
                    ...
                    // 数据分解
                    var user_openid=userJson.openid;
                    var user_nickname=userJson.nickname;
                    var user_sex=userJson.sex;
                    var user_language=userJson.language;
                    var user_city=userJson.city;
                    var user_province=userJson.province;
                    var user_country=userJson.country;
                    var user_headimgurl=userJson.headimgurl;
                    var user_privilege=userJson.privilege;
                    var user_unionid=userJson.unionid;


                    //提交数据信息
                    $.ajax({
                      type:"post",
                      url:"http://180.76.243.205:8383/_API/_user/wxlogin",
                      data:{
                        openid:user_openid,
                        ...
                        init_time:userDataTimer
                      },
                      success:function(data){
                        commFinish();
                        // alert(data.code);
                        // alert(data.code=="E0000");
                        // alert(JSON.stringify(data));
                        if(data.code=="E0000"){
                          //数据保存
                          // ponyUserData=data.data.userInfo;
                          ...
                          // $location.path("/userSettingTel/"+data.data.userInfo.id);
                          window.location.href="index.html#/userSettingTel/"+data.data.id;
                        }else if(data.code=="E8888"){
                          commFinish();
                          //数据保存
                          ponyUserData=data.data.userInfo;
                          ponyUserCar=data.data.userCar_info;
                          localStorage["ponyUserName"]=data.data.userInfo.phone;
                          localStorage["ponyUserPwd"]=data.data.userInfo.password;
                          sessionStorage["ponyUserToken"]=data.data.userInfo.token;
                          localStorage['userDataTimer']=data.data.init_time;
                          $rootScope.addressCity="";
                          // $location.path("/main");
                      ...
                      error:function(){
                        commFinish();
                        alertMsg("确定","数据失效，请得重新授权",function(){});
                      }
                    });
            ...
            commFinish();
            alertMsg("确定","微信登陆失败:"+reason,function(){});
          });
  ...
  //用户手机号设置页面
  horseApp.controller("userSettingTelCtrl",function($scope,$http,$location,$routeParams,$rootScope){
    var token=sessionStorage["ponyUserToken"];
    ...
    $scope.changeTelSubmit=function(tel,verify){
      ...
      $http({
        method:"post",
        url:"http://180.76.243.205:8383/_API/_user/bindPhone",
        data:"user_id="+userId+"&token="+token+"&phone="+tel+"&code_id="+verifyCodeId+"&code="+verify
      }).success(function(data){ 
        // console.log(data)
        if(data.code=="E0000"){
          commFinish();
  ...
  horseApp.controller("userAmountCtrl",function($scope,$http,$location){
    ...
    $scope.userCreditAmount=function(){
      $(".alertMsg>.msgBody>.detail").css({
        lineHeight:'24px',
        fontSize:16,
        textAlign:'left'
      });
      alertMsg("确定","可用额度 = 默认车辆剩余额度 + 马粮余额 ，充值后会优先充满剩余额度，当剩余额度 = 信用额度时，即可充值马粮余额。",function(){
        $(".alertMsg>.msgBody>.detail").removeAttr("style");
      }); 
    }
    // 剩余信用额度
    ...
    //马粮充值
    $scope.mlRecharge=function(){

      // console.log($('#userAmountPage>.payNumber>ul>li.active').index());
      // console.log($('#userAmountPage>.payMode>.active').index());
      //充值金额选中项
      var selRechargeVal=$('#userAmountPage>.payNumber>ul>li.active').index();
      console.log(payList);
      //充值金额
      var rechargeVal=0;
      if(selRechargeVal<4){
        rechargeVal=payList[selRechargeVal].id;
      }else{
        rechargeVal=$("#otherRechargeVal").val();
        if(!(/^[1-9][0-9]{0,}$/.test(rechargeVal))){
          alertMsg("确定","请输入整数的充值金额",function(){});
          return;
        }
      }
      //充值方式
      var rechargeMode=$('#userAmountPage>.payMode>.active').index();

      //验证
      if(rechargeVal<0){
        alertMsg("确定","请输入正确的充值金额",function(){});
        return;
      }
      if(rechargeMode==-1){
        alertMsg("确定","请选择充值方式",function(){});
        return;
      }


      //初始化传参
      var fromObj_id=selRechargeVal<4?rechargeVal:'custorm';
      var formSale_ids=selRechargeVal<4?payList[selRechargeVal].recharge_value:rechargeVal;

      // console.log(rechargeVal,rechargeMode);
      switch(rechargeMode){
        case 1:
          //银联支付
          alertMsg("确定","正在开发，敬请期待",function(){});
        ...
        case 2:
          ...
          Wechat.isInstalled(function (installed) {
            ...
            if(WEIXININSTALL){
              ...
              $.ajax({
                type:'post',
                url:'http://180.76.243.205:8383/_API/_wx/pay',
                data:{
                  user_id:ponyUserData.id,
                  token:token,
                  obj_id:fromObj_id,
                  trade_mode:'recharge',
                  sale_ids:formSale_ids,
                  cxwy:2
                },
                success:function(ponyData){
                  // alert(JSON.stringify(ponyData));

                  if(ponyData.code=="E0000"){
                    //调用微信支付，params微信支付配置参数
                    var params = {
                      partnerid: parseInt(ponyData.data.partnerid), // merchant id
                      prepayid: ponyData.data.prepayid, // prepay id
                      noncestr: ponyData.data.noncestr, // nonce
                      timestamp: ponyData.data.timestamp, // timestamp
                      sign: ponyData.data.sign, // signed string      
                    };

                    // alert(JSON.stringify(params));
                    var payObj_id=ponyData.data.obj_id;
                    ...
                    Wechat.sendPaymentRequest(params, function (data) {
                      ...
                      $.ajax({
                        type:'post',
                        url:'http://180.76.243.205:8383/_API/_wx/success',
                        data:{
                          user_id:ponyUserData.id,
                          token:token,
                          code:'SUCCESS',
                      ...
                      commFinish();
                      // alert("Failed: " + reason);
                      alertMsg("确定","充值失败",function(){});
                    });


                  }else if(ponyData.code=="E0014"){
                    commFinish();
                    alertMsg("确定",data.message,function(){
                      window.location.href="index.html#/login";
                    });
          ...
          $.ajax({
            ...
            success:function(data){
              ...
              if(data.code=="E0000"){
                ...
                var payObj_id=data.data.obj_id;
  ...
  horseApp.controller('userSetNewPayPwdCtrl',function($scope,$http,$location){
    ...
    $scope.submitPayPwdFirst=function(payPwd){
      ...
      if(!pwd){
        ...
        alertMsg("确定","密码由六位数字组成",function(){
  ...
  horseApp.controller("userExpandCtrl",function($scope,$http,$location){
    ...
    // 推广奖励文案
    $scope.expandAwardDetail="";
  
    // 推广方式文案
    $scope.expandModelDetail="";

    //获取数据 
    $http({
      method:"post",
      url:"http://180.76.243.205:8383/_API/_userShare/get",
      data:"user_id="+ponyUserData.id+"&token="+token+"&share_id="+1
    }).success(function(data){
      if(data.code=="E0000"){
        console.log(data);
        expandLink.tit=data.data.title;
        expandLink.content=data.data.content;
        expandLink.img=data.data.img;
        expandLink.link=data.data.url;
        $scope.expandAwardDetail=data.data.award;
        $scope.expandModelDetail=data.data.rule;
    ...
    $scope.wxShareFriend=function(){
      ...
      Wechat.isInstalled(function (installed) {
        ...
        if(WEIXININSTALL){
          ...
          Wechat.share({
            message:{
              title:expandLink.tit,
              description:expandLink.content,
              //这是分享图片
              thumb:expandLink.img,
              media:{
                type:Wechat.Type.LINK,
                webpageUrl:expandLink.link
              }    
            },
  ...
  //小马驾驾联系客服页面
  horseApp.controller("contactServiceCtrl",function($scope,$http,$location){
    $scope.height=vHeight;
    ...
    // console.log(vWidth);
    var imgTit=vWidth*0.6*1.7292|0;
    var imgFoot=vWidth*0.4937|0;
    // console.log((vHeight-90-(imgTit+imgFoot))/2);
    $scope.gap=(vHeight-50-(imgTit+imgFoot))/2<20?20:(vHeight-50-(imgTit+imgFoot))/2;
  ...
  //畅行无忧页面
  horseApp.controller('cxwyServerCtrl',function($scope,$http,$location){
    $scope.height=vHeight;
    ...
    $http({
      method:'post',
      url:'http://180.76.243.205:8383/_API/_userCar/getDefaultCxwy',
      data:"user_id="+ponyUserData.id+"&token="+token
    }).success(function(data){
      if(data.code=="E0000"){
        console.log(data.data);
        $scope.cxwyServer=data.data.cxwy;
      }else if(data.code=="E0014"){
        alertMsg("确定",data.message,function(){
          window.location.href="index.html#/login";
        });
  ...
  horseApp.controller("userBolgConfigCtrl",function($scope,$routeParams,$http){
    ...
    $scope.$watch("$viewContentLoaded",function(){
      ...
      if(userBolg!="null"){
        ...
        // console.log(ponyUserCar.length||false);
        if(ponyUserCar||false){
          $(".referrer").css("display","none");
        }
      }
    ...
    $scope.submitBolg=function(){
      ...
      if(userBolg=="null"){
        ...
        if(!userDriveID){
          alertMsg("确定","请选择您的行驶证日期",function(){}); 
          return;
        }else if(userDriveID>formatNow){
          // console.log(userDriveID,formatNow);
          alertMsg("确定","您选择的行驶证日期不正确",function(){}); 
          return;
        }else if(new Date(userDriveID).getFullYear()+15<new Date().getFullYear()){
          // console.log(new Date(userDriveID).getFullYear()+15,new Date().getFullYear());
          alertMsg("确定","您选择的行驶证日期不正确",function(){}); 
          return;
        }
  ...
  horseApp.controller("checkoutCounterCtrl",function($scope,$http,$location,$routeParams){
    ...
    $http({
      ...
      if(data.code=="E0000"){
        ...
        //对重复的优惠券进行去重
        var salesArr=data.data.sales;
        var salesNewArr=[];
        for(var i=0;i<salesArr.length;i++){
          var one=salesArr[i];
          // console.log(one);
          // console.log(searchArr(salesArr,'view_type_id',one.view_type_id));
          if(!searchArr(salesNewArr,'view_type_id',one.view_type_id).result){
            salesNewArr.push(one);
          }
        }

        $scope.userCouponList=salesNewArr;
              ...
              $.ajax({
                ...
                success:function(ponyData){
                  ...
                  if(ponyData.code=="E0000"){
                    ...
                    // alert(JSON.stringify(params));
                    var ponyOrderId=ponyData.data.obj_id;
                    Wechat.sendPaymentRequest(params, function (data) {
                      // commFinish();
                      // alert("Success");
                      // alert(JSON.stringify(data));
  ...
  horseApp.controller("changeTireSelCtrl",function($scope,$http,$location,$rootScope){
    ...
    $http({
      ...
      data:"user_id="+ponyUserData.id+"&token="+token+"&longitude="+(deviceLocation.state==1?deviceLocation.longitude:lonBD)+"&latitude="+(deviceLocation.state==1?deviceLocation.latitude:latBD)+"&city_name="+(addressCity?addressCity:cityBD)
    }).success(function(data){
      if(data.code=="E0000"){
        // console.log(data.data);
        ...
        isShop=data.data.store_exsist;
        ponyShopRegionId=data.data.position_id;
      }else if(data.code=="E0014"){
        alertMsg("确定",data.message,function(){
          window.location.href="index.html#/login";
        });
    ...
    $scope.tireOriginalChange=function(){
      //除了对用户的默认车辆是否有过默认下单，且所选城市有店铺
      if(userOrderLength>'0'){
        if(isShop==1){
          // $location.path("/originalChangeTire/1");
          $location.path("/originalChangeTire/"+ponyShopRegionId);
        }else{
          alertMsg("确定","您所在的地区无小马服务门店",function(){}); 
        }
      }else{
    ...
    $scope.tireRepairServer=function(){
      ...
      if(userOrderLength>'0'){
        if(isShop==1){
          $location.path("/tireRepair/"+ponyShopRegionId);
        }else{
          alertMsg("确定","您所在的地区无小马服务门店",function(){}); 
        }
      }else{
    ...
    $scope.tireFreeChange=function(){
      ...
      if(userOrderLength>'0'){
        if(isShop==1){
          // $location.path("/ponyTireChange/1");
          $location.path("/ponyTireChange/"+ponyShopRegionId);
        }else{
          alertMsg("确定","您所在的地区无小马服务门店",function(){}); 
        }
      }else{
  ...
  //轮胎原厂更换服务页面
  horseApp.controller("originalChangeTireCtrl",function($scope,$http,$location,$routeParams){
    $scope.height=vHeight;
    ...
    $scope.$watch("$viewContentLoaded",function(){
      //页面从头
      $('body,html').scrollTop(0);
      console.log(deviceLocation.longitude,lonBD,deviceLocation.latitude,latBD);
      //开始加载通讯
      commStart();
      ...
      $http({
        ...
        data:"user_id="+ponyUserData.id+"&token="+token+"&longitude="+(deviceLocation.state==1?deviceLocation.longitude:lonBD)+"&latitude="+(deviceLocation.state==1?deviceLocation.latitude:latBD)+"&position_id="+cityId     
        // data:"user_id="+ponyUserData.id+"&token="+token+"&longitude="+120.439677+"&latitude="+36.167091
      }).success(function(data){
        commFinish();
        if(data.code=="E0000"){
          console.log(data.data);
        ...
        }else if(data.code=="E0109"){
          alertMsg("确定","您所在地区没有小马服务门店",function(){
            clearInterval(bannerTimer);
            window.location.href="index.html#/changeTireSel";
          });
        }else if(data.code=="E0014"){
          alertMsg("确定",data.message,function(){
            clearInterval(bannerTimer);
  ...
  //轮胎服务，免费补胎页面
  horseApp.controller("tireRepairCtrl",function($scope,$http,$location,$routeParams){
    $scope.height=vHeight;
    ...
    $http({
      ...
      data:"user_id="+ponyUserData.id+"&token="+token+"&longitude="+(deviceLocation.state==1?deviceLocation.longitude:lonBD)+"&latitude="+(deviceLocation.state==1?deviceLocation.latitude:latBD)+"&position_id="+cityId  
      // data:"user_id="+ponyUserData.id+"&token="+token+"&longitude="+1.000000+"&latitude="+1.000000
    }).success(function(data){
      console.log(data);
      if(data.code=="E0000"){
        //修补规则
  ...
  //轮胎服务小马架架轮胎更换页面
  horseApp.controller("ponyTireChangeCtrl",function($scope,$http,$location,$routeParams){
    $scope.height=vHeight;
  ...
  horseApp.controller("shopListCtrl",function($scope,$rootScope,$location,$http,$routeParams){
    ...
    function getShopList(key){
      ...
      $http({
        method:"post",
        url:"http://180.76.243.205:8383/_API/_store/getList",
        data:"user_id="+ponyUserData.id+"&token="+token+"&longitude="+(deviceLocation.state==1?deviceLocation.longitude:lonBD)+"&latitude="+(deviceLocation.state==1?deviceLocation.latitude:latBD)+"&service_type="+typeId+(key?"&key_words="+key:'')+"&city_name="+($scope.addressCity?$scope.addressCity:cityBD)
      }).success(function(data){
        commFinish();
        console.log(data);
        if(data.code=="E0000"){
    ...
    $scope.$watch("$viewContentLoaded",function(){
      ...
      $("#selShopFiltrate>.subMenu>.filtrateBody>.filtrateBtn>.filtrateSet").on("click",function(){
        ...
        if(isUpkeep){
          for(var i=0;i<newShopList.length;i++){
            var one=newShopList[i];
            // console.log(one.id,one.service_type.service_type_ii);
            if(one.service_type.service_type_ii==2){
              // newShopList.splice(i-delI,1);
              newShopList.splice(i,1);
              i--;
